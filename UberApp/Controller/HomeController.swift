    ...
    var user: User? {
        didSet{
            ...
            if user?.accountType == .passenger{
                fetchDrivers()
                configureLocationInputActvationView()
                observeCurrentTrip()
                configureSaveUserLocations()
            } else {
                observeTrips()
            }
        }
...
//            print("DEBUG: Show pickup passenger controller..")
    ...
    func configureSaveUserLocations(){
        guard let user = user else { return }
    ...
    func configureLocationInputActvationView(){
